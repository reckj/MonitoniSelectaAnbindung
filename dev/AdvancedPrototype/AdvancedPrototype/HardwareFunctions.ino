...
static void buttonSwitchHandler(uint8_t btnId, uint8_t btnState) {
  if (btnState == BTN_PRESSED) {
    buttonPushedState = true;
    timerButtonPress.stop();
    return;
  }

  // btnState == BTN_OPEN.
  buttonPushedState = false;
  timerButtonPress.stop();
  timerButtonPress.start();
}
...
static Button buttonSwitch(0, buttonSwitchHandler);
...
static void updateInputs() {
  ...
  buttonSwitch.update(digitalRead(Button_PIN));
